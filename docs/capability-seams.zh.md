...
| `ctx.apiProxy` | `core` | `apiproxy` | - | `connection`, [`desktop-app`](../packages/bundle/desktop-app) | - | 与传输无关的 Host 网关接口：它分派浏览器 API 调用，每条打开的 Host 流自行订阅转发事件，而不是由广播方法向其推送。 |
| `ctx.desktopRuntime` | `bundle` | [`desktop-app`](../packages/bundle/desktop-app) | - | - | - | 为受监管的桌面 Host 进程物化统一 Host fetch handler 与事件流；它不打开任何网络监听端口。 |
